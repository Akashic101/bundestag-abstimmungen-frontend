...
function SpecificMember() {
    ...
    useEffect(() => {
        if (aktuelleFraktion) {
            fetch(`http://localhost:3000/fraktiongruppe/${encodeURIComponent(aktuelleFraktion)}/mitglieder`)
                .then(response => {
                    if (!response.ok) {
                        throw new Error('Network response was not ok');
                    }
                ...
                .then(data => {
                    ...
                    setAktuellesMitglied(bezeichnungArray[Math.floor(Math.random()*bezeichnungArray.length)])
                })
                .catch(error => {
                    console.error('There was a problem with the fetch operation:', error);
                });
    ...
    useEffect(() => {
        if (aktuelleFraktion) {
            fetch(`http://localhost:3000/mitglieder/${encodeURIComponent(aktuellesMitglied)}`)
            .then(response => {
                if (!response.ok) {
                    throw new Error('Network response was not ok');
                }
            ...
            .then(data => {
                ...
                data.forEach(item => {
                    const keys = ['ja', 'nein', 'Enthaltung', 'ungültig', 'nichtabgegeben'];
                    keys.forEach(key => {
                        if (item[key]) {
                            let keyName = key;
                ...
                const transformedData = Object.entries(chartDataMap).map(([name, value]) => ({
    ...
    return (
        <Grid>
            ...
            <Grid.Col span={4}> 
                <DonutChart paddingAngle={2} withLabelsLine withLabels chartLabel={aktuellesMitglied} data={donutChartData} />
            </Grid.Col>
        </Grid>
    );
}
